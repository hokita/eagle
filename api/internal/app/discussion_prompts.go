...
func buildSummaryPrompt(q *DiscussionQuestion, transcript []DiscussionMessage, reflectionJA string) string {
	...
	b.WriteString("test: would a friend say it to you in ordinary conversation today? Use a connector ")
	b.WriteString("or softener only where the learner's own words already carry it — \"so\" only where ")
	b.WriteString("they gave that reason, \"for a while\" only where they said it had been a while, ")
	b.WriteString("\"kind of\" only where they were hedging, never on an opinion they stated flatly. ")
	b.WriteString("They are wording, not content: never let one add a fact, a timeframe, or a degree ")
	b.WriteString("of conviction the learner did not express.\n")
